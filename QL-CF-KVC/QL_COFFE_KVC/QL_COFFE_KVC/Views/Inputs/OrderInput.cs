...
{
    public partial class OrderInput : Form
    {
        ...
        public OrderInput(DataRow itemRow, DataTable detail)
        {
            Init();
            if (itemRow == null) return;
            txtCode.Text = itemRow["ma_dh"].ToString();
            txtName.Text = itemRow["ten_khach"].ToString();
            cbTable.SelectedValue = itemRow["ma_ban"];
        ...
        void OnSaveClicked(object sender, EventArgs e)
        {
            SaveClicked?.Invoke(this, new DataSetArgs(new DataSet().AddTable(new DataTable()
                ...
                .AddRowValue("ma_ban", cbTable.SelectedValue)
        ...
        {
            var detail = new OrderDetailInput(FormAction.New);
        ...
        void OnEditClicked(object sender, EventArgs e)
        {
            ...
            var detail = new OrderDetailInput(FormAction.New, rowView.Row);
        ...
        void Init()
        {
            ...
            cbStatus.DataSource = OrderStatusDataSource.Instance;
            cbTable.DataSource = FormatTable(DataCenter.Instance.ExecuteDataTable("Table", "TTTable", new DataSet().AddTable(new DataTable().AddRowValue(0, "tinh_trang", "*").AddRowValue(0, "trang_thai", "1")), 0));
        }

        DataTable FormatTable(DataTable dataTable)
        {
            foreach (DataRow vRow in dataTable.Rows)
            {
                var appendText = AvaiableTableDataSource.Instance.Find(x => x.Value.ToString() == vRow["tinh_trang"].ToString())?.Name;
                if (!string.IsNullOrEmpty(appendText))
                {
                    vRow["ten_ban"] = $"{vRow["ten_ban"]} ({appendText})";
                }
            }

            var row = dataTable.NewRow();
            row["ma_ban"] = "";
            row["ten_ban"] = "Bán mang đi";
