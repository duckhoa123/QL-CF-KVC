...
{
    public class FormPlaces : FormBase
    {
        protected override string Controller => "Place";
        protected override void LoadData()
        {
            ViewGrid = DataCenter.Instance.ExecuteDataTable(Controller, "Places", Utils.DataFromArray(new Param("trang_thai", "*")), 0);
        ...
        protected override void InitColumns()
        {
            ...
            GridView.Columns["ten_khu"].HeaderText = "Tên khu";
        ...
        protected override void OnEditClicked(object sender, EventArgs e)
        {
            ...
            var itemInput = new PlaceInput(DataCenter.Instance.ExecuteDataRow(Controller, "PlaceByID", Utils.DataFromArray(new Param("ma_khu", row.Cells["ma_khu"].Value)), 0));
        ...
        {
            if (GetSelectionRow() is not DataGridViewRow row) return;
            DataCenter.Instance.Execute(Controller, "DeletePlace", Utils.DataFromArray(new Param("ma_khu", row.Cells["ma_khu"].Value)), 0);
            ViewGrid.Rows.RemoveAt(ViewGrid.Rows.IndexOf(ViewGrid.Select($"ma_khu = '{row.Cells["ma_khu"].Value}'")[0]));
        }

        void OnInsert(object sender, DataSetArgs e)
        {
            if (sender is not PlaceInput input) return;
            var row = DataCenter.Instance.ExecuteDataRow(Controller, "InsertPlace", e.Value, 0, 1, 2);
            if (row == null)
                return;
        ...
        void OnEdit(object sender, DataSetArgs e)
        {
            ...
            oldTable.AddRowValue("ma_khu_old", selectRow.Cells["ma_khu"].Value);
            oldTable.AddRowValue("ngay_sua_old", selectRow.Cells["ngay_sua"].Value, typeof(DateTime));
            e.Value.Tables.Add(oldTable);

            var row = DataCenter.Instance.ExecuteDataRow(Controller, "EditPlace", e.Value, 0, 1, 2);
            ...
            ViewGrid.UpdateRow(ViewGrid.Rows.IndexOf(ViewGrid.Select($"ma_khu = '{selectRow.Cells["ma_khu"].Value}'")[0]), row);
