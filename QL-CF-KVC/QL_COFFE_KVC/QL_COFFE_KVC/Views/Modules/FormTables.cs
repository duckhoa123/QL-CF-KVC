...
{
    public class FormTables : FormBase
    {
        protected override string Controller => "Table";
        protected override void LoadData()
        {
            ViewGrid = DataCenter.Instance.ExecuteDataTable(Controller, "Tables", Utils.DataFromArray(new Param("trang_thai", "*")), 0);
            SetDataSource(ViewGrid);
        }

        protected override void InitColumns()
        {
            GridView.Columns["ma_ban"].HeaderText = "Mã bàn";
            GridView.Columns["ten_ban"].HeaderText = "Tên bàn";
            GridView.Columns["size"].HeaderText = "Số lượng tối đa";
            GridView.Columns["tinh_trang"].HeaderText = "Tình trạng";
            GridView.Columns["trang_thai"].HeaderText = "Trạng thái";
            GridView.Columns["ma_khu"].HeaderText = "Mã khu";
        ...
        protected override void OnEditClicked(object sender, EventArgs e)
        {
            ...
            var itemInput = new TableInput(DataCenter.Instance.ExecuteDataRow(Controller, "TableByID", Utils.DataFromArray(new Param("ma_ban", row.Cells["ma_ban"].Value)), 0));
        ...
        {
            if (GetSelectionRow() is not DataGridViewRow row) return;
            DataCenter.Instance.Execute(Controller, "DeleteTable", Utils.DataFromArray(new Param("ma_ban", row.Cells["ma_ban"].Value)), 0);
            ViewGrid.Rows.RemoveAt(ViewGrid.Rows.IndexOf(ViewGrid.Select($"ma_ban = '{row.Cells["ma_ban"].Value}'")[0]));
        }

        void OnInsert(object sender, DataSetArgs e)
        {
            if (sender is not TableInput input) return;
            var row = DataCenter.Instance.ExecuteDataRow(Controller, "InsertTable", e.Value, 0, 1, 2);
            if (row == null)
                return;
        ...
        void OnEdit(object sender, DataSetArgs e)
        {
            ...
            oldTable.AddRowValue("ma_ban_old", selectRow.Cells["ma_ban"].Value);
            oldTable.AddRowValue("ngay_sua_old", selectRow.Cells["ngay_sua"].Value, typeof(DateTime));
            e.Value.Tables.Add(oldTable);

            var row = DataCenter.Instance.ExecuteDataRow(Controller, "EditTable", e.Value, 0, 1, 2);
            ...
            ViewGrid.UpdateRow(ViewGrid.Rows.IndexOf(ViewGrid.Select($"ma_ban = '{selectRow.Cells["ma_ban"].Value}'")[0]), row);
