...
{
    public class FormOrders : FormBase
    {
        protected override string Controller => "Order";
        protected override void LoadData()
        {
            ViewGrid = DataCenter.Instance.ExecuteDataTable(Controller, "Orders", Utils.DataFromArray(new Param("trang_thai", "*"), new Param("xtype", "0")), 0);
            SetDataSource(ViewGrid);
        }

        protected override void InitColumns()
        {
            GridView.Columns["ma_dh"].HeaderText = "Mã đơn hàng";
        ...
        protected override void OnEditClicked(object sender, EventArgs e)
        {
            ...
            var itemInput = new OrderInput(ds.Tables[0].Rows[0], ds.Tables[1]);
        ...
        protected override DataSet GetSearchFilter()
        {
            ...
            result.Tables[0].AddRowValue(0, "xtype", "0");
            return result;
        }

        void OnInsert(object sender, DataSetArgs e)
        {
            if (sender is not OrderInput input) return;
